...
EWeaponTier AItemDropHandle::GetItemTier(int32 maxTier)
{
	...
	if (TierValue <= 60) return EWeaponTier::Normal; 
	else if (TierValue <= 80) return EWeaponTier::Rare; 
	else if (TierValue <= 95) return EWeaponTier::Epic; 
	else if (TierValue <= 99) return EWeaponTier::Unique; 
	else if (TierValue <= 100) return EWeaponTier::Legend; 
